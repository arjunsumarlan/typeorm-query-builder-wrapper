...
> {
  ...
  /**
   * Apply Query Builder to Entity with Alias
   *
   * @param entityType EntityTarget<Entity>
   * @param entityAlias Alias
   * @return {*} {QueryBuilder}
   */
  public applyEntity<Entity>(
  ...
  private async applyManyAggregate(
    ...
    for (const propertySelector of args) {
      ...
      if (args.indexOf(propertySelector) === 0) {
        cloneQB.select(`${type}(${transformedProp})`, `${prop}Sum`);
      } else {
        cloneQB.addSelect(`${type}(${transformedProp})`, `${prop}Sum`);
      }
    }

    return await cloneQB
      .groupBy()
      .orderBy()
      .limit(UNDEFINED_CONST)
      .offset(UNDEFINED_CONST)
  ...
  public async getManySum(
    ...
    return this.applyManyAggregate(args, 'SUM');
  }
  /**
  ...
  public async getManyAverage(
    ...
    return this.applyManyAggregate(args, 'AVG');
  }
  /**
   * Gets max of field.
   *
  ...
  public async getManyMax(
    ...
    return this.applyManyAggregate(args, 'MAX');
  }
  /**
   * Gets min of field.
   *
  ...
  public async getManyMin(
    ...
    return this.applyManyAggregate(args, 'MIN');
  }
}
